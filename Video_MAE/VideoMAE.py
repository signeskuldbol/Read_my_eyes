...
lr_encoder_last = 2e-5        # last N_last_layers(1/3 of total num layers) layers by default (can be higher as those layers are more task-specific)
...
fraction_last = 0           # last % of layers get lr_encoder_last, rest get lr_encoder_rest
#########################
...
def make_optimizer(m):
    ...
    N_last_layers = max(1, int(round(num_layers * fraction_last)))  # e.g., last third
...
fig.tight_layout()
fig.savefig(cm_path, bbox_inches="tight")   # overwrite each run
